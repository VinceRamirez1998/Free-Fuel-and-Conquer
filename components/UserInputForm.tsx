...
import type { FormData, UserData, Language } from '../types';
import { 
    GOAL_OPTIONS, 
...
const UserInputForm: React.FC<UserInputFormProps> = ({ initialData, onSubmit }) => {
    ...
    const toggleLanguage = () => {
        setFormData(prev => ({ ...prev, language: prev.language === 'en' ? 'es' : 'en' }));
    };
    
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
        const { name, value } = e.target;
        const parsedValue = (name === 'age' || name.startsWith('weight') || name.startsWith('height')) && value !== '' ? parseInt(value, 10) : value;
        ...
        <form onSubmit={handleSubmit} className="space-y-8 bg-white p-6 md:p-8 rounded-lg shadow-lg relative">
            ...
            <div className="absolute top-4 right-4 md:top-8 md:right-8 z-10">
                <button
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 border border-slate-300 rounded-full hover:bg-slate-200 transition-colors"
                >
                    <span className={`text-sm font-bold ${language === 'en' ? 'text-slate-800' : 'text-slate-400'}`}>EN</span>
                    <span className="text-slate-300">|</span>
                    <span className={`text-sm font-bold ${language === 'es' ? 'text-slate-800' : 'text-slate-400'}`}>ES</span>
                </button>
            </div>
